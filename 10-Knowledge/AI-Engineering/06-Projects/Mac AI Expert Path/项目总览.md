...
如果目标是真正掌握模型、训练、微调和部署，而不是“调一下 LLM API”，从这 3 页开始：

1. [[Mac AI 深度实战学习总纲]]：定义完整能力地图。
2. [[Mac AI 深度实战 16 周路径]]：把能力拆成每周实验和产物。
3. [[第 1 周启动任务：模型与本地推理闭环]]：本周就能开始执行。

这条深度版路线会把原来的 `E0-E12` 实验扩成更明确的主线：

```text
模型原理 -> 训练循环 -> 本地推理 -> 量化 -> MLX -> 数据集 -> LoRA/SFT -> Eval -> RAG/Agent -> Serving -> 云上映射
```
